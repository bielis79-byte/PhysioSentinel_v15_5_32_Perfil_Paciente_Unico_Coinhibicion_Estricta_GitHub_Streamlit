...
st.caption("v15.5.8 · Atractores animados en pestaña 4 · Persistent Cloud Architecture II · Narrativa predictiva Presente → Tendencia → Probabilidad · Control Autonómico doble plano separado: Actual + Longitudinal V/S/L/C/R · Nivel 2 XGBoost")
...
def _norm_centered_v15528(value, low, high, optimal=False):
    """Convierte un rango normativo a escala aproximadamente [-1.5, +1.5].
    0 = centro del rango; ±0.5 ≈ bordes; fuera del rango aumenta gradualmente.
    """
    try:
        v = float(value)
        if not np.isfinite(v) or high <= low:
        ...
        z = (v - mid) / half
        if optimal:
            return float(np.clip(1.0 - abs(z), -1.5, 1.0))
...
def _autonomic_abs_vector_levels_v15528(row, age_years=None, sex="No especificado"):
    """V/S/L/C/R actuales contextualizados por edad/sexo, 100% transversales."""
    ctx = _autonomic_age_context_scores_v15528(row, age_years=age_years, sex=sex)
    s = ctx["scores"]
    get = lambda k: _autonomic_num(row.get(k, np.nan))

    # Vagal: métricas clásicas vagales, con pesos próximos a la matriz original.
    ...
    # S: barorreflejo/alerta, no "simpático puro".
    # LF absoluto contextualizado + LF/HF + FC + DFAα1, y dominancia LF relativa.
    rel_lf = np.nan
    ...
    S = _autonomic_weighted_score([
        (s.get("LF", np.nan), 0.45),
        (s.get("LF_HF", np.nan), 0.35),
    ...
    # Reserva/amplitud: ahora usa SOLO referencias por edad.
...
def _autonomic_classify_instantaneous_v15531(
    ...
    """v15.5.31 · Topología multidimensional con coherencia fisiológica.

    Mantiene la participación formal V/S/L/C/R de v15.5.30, pero añade una
    capa previa de coherencia fisiológica para impedir que un score agregado
    produzca Coinhibición cuando el registro actual muestra actividad
    multibanda y organización conservadas.

    100% transversal:
    - NO usa Z longitudinales.
    - La actividad multibanda actual se obtiene de HF/LF/VLF/TOTAL
      contextualizados por edad/sexo.
    """
    ...
    finite = [np.isfinite(x) for x in vals]
    reserve = _reserve_context_label_v15528(R_abs, None)

    if sum(finite) < 3 or not (np.isfinite(V_abs) and np.isfinite(S_abs)):
        return {
            "topologia": "Desacoplamiento",
            "calificador_topologico": "información multidimensional insuficiente",
            "topologia_mostrada": "Desacoplamiento · información multidimensional insuficiente",
            "predominio": "No determinado",
            "reserva": reserve,
            "estado": "Desacoplamiento · información multidimensional insuficiente",
            "predominio_vectorial": "No determinado",
            "apoyo_espectral": "Evidencia insuficiente",
            "discordancia_espectral": False,
            "HF_DOM_actual": hfdom, "LF_DOM_actual": lfdom, "LF_HF_actual": lfhf,
            "Amplitud_absoluta_idx": idx_amp,
            "Patron_oscilatorio_actual_apoyo": osc_pat,
            "Nota_convergencia_actual": "No hay suficientes dimensiones actuales para una clasificación robusta.",
            "Score_Coactivacion": np.nan, "Score_Reciprocidad": np.nan,
            "Score_Coinhibicion": np.nan, "Score_Coinhibicion_bruto": np.nan, "Score_Desacoplamiento": np.nan,
            "Contribucion_VS_pct": 55, "Contribucion_LCR_pct": 45,
            "Actividad_multibanda_actual": False,
            "Coinhibicion_permitida": False,
            "Motivo_compuerta_coinhibicion": "Información insuficiente",
        }

    # ------------------------------------------------------------
    # Evidencia espectral actual y predominio relativo.
    # ------------------------------------------------------------
    ...
    alert_spec = bool(
        lf_gt_hf and (
            (np.isfinite(lfhf) and lfhf > 1.5)
            or (np.isfinite(lfdom) and lfdom > 40)
        )
    )

    spectral_delta = float(hfdom - lfdom) if np.isfinite(hfdom) and np.isfinite(lfdom) else np.nan
    strong_vagal_spec = bool(
        vagal_spec and (
            (np.isfinite(spectral_delta) and spectral_delta >= 10.0)
            or (np.isfinite(hfdom) and hfdom >= 45.0 and hf_gt_lf)
        )
    )
    strong_alert_spec = bool(
        alert_spec and (
            (np.isfinite(spectral_delta) and spectral_delta <= -10.0)
            or (np.isfinite(lfdom) and lfdom >= 45.0 and lf_gt_hf)
        )
    )

    # ------------------------------------------------------------
    # Actividad multibanda ACTUAL contextualizada por edad/sexo.
    # Los scores contextuales están centrados: ~0 = centro del rango etario.
    # ------------------------------------------------------------
    band_scores = {
        k: context_scores.get(k, np.nan)
        for k in ("HF", "LF", "VLF", "TOTAL")
    }
    finite_band_scores = [float(v) for v in band_scores.values() if np.isfinite(v)]
    n_band_available = len(finite_band_scores)
    n_not_reduced = sum(v >= -0.25 for v in finite_band_scores)
    n_preserved_or_high = sum(v >= 0.0 for v in finite_band_scores)

    total_ctx = band_scores.get("TOTAL", np.nan)
    multiband_current = bool(
        n_band_available >= 3
        and n_not_reduced >= 2
        and (not np.isfinite(total_ctx) or total_ctx >= -0.50)
    )
    multiband_strong = bool(
        n_band_available >= 3
        and n_preserved_or_high >= 2
        and (not np.isfinite(total_ctx) or total_ctx >= -0.25)
    )

    # ------------------------------------------------------------
    # Scores topológicos V/S/L/C/R.
    # ------------------------------------------------------------
    pos = lambda x: float(np.clip((x + 1.0) / 2.0, 0.0, 1.0)) if np.isfinite(x) else 0.5
    neg = lambda x: float(np.clip((1.0 - x) / 2.0, 0.0, 1.0)) if np.isfinite(x) else 0.5

    pv, ps, pl, pc, pr = [pos(x) for x in vals]
    nv, ns, nl, nc, nr = [neg(x) for x in vals]

    vs_coact = (pv + ps) / 2.0
    vs_coinhib = (nv + ns) / 2.0
    vs_recip = min(1.0, abs(V_abs - S_abs) / 1.25)

    lcr_functional = 0.25*pl + 0.40*pc + 0.35*pr
    lcr_reduced = 0.25*nl + 0.35*nc + 0.40*nr

    arr = np.array([x for x in vals if np.isfinite(x)], dtype=float)
    dispersion = float(np.clip(np.std(arr) / 0.85, 0.0, 1.0))
    sign_conflict = float(np.mean(np.sign(arr) != np.sign(np.nanmedian(arr)))) if len(arr) else 0.0

    score_coact = 0.55*vs_coact + 0.45*lcr_functional
    score_coinhib = 0.55*vs_coinhib + 0.45*lcr_reduced
    score_recip = 0.55*vs_recip + 0.45*lcr_functional
    score_disc = 0.60*dispersion + 0.40*sign_conflict

    # ------------------------------------------------------------
    # COMPUERTA FISIOLÓGICA DE COINHIBICIÓN
    # Coinhibición exige reducción convergente y NO actividad multibanda
    # conservada/alta. El simple signo negativo de V/S no es suficiente.
    # ------------------------------------------------------------
    v_low = np.isfinite(V_abs) and V_abs < -0.20
    s_low = np.isfinite(S_abs) and S_abs < -0.20
    r_low = np.isfinite(R_abs) and R_abs < -0.35
    c_low = np.isfinite(C_abs) and C_abs < -0.25

    coinhibition_allowed = bool(
        v_low and s_low and r_low and c_low
        and not multiband_current
        and not strong_vagal_spec
        and not strong_alert_spec
    )

    _score_coinhib_raw_v15532 = float(score_coinhib)
    if not coinhibition_allowed:
        # v15.5.32: una compuerta fisiológica debe ser realmente excluyente.
        # Conservamos el score bruto para auditoría, pero Coinhibición NO puede
        # ser la familia ganadora si no cumple sus criterios fisiológicos.
        score_coinhib = -1.0

    # Actividad multibanda + complejidad/reserva no reducidas favorecen
    # coactivación funcional.
    functional_support = bool(
        multiband_current
        and (
            (np.isfinite(C_abs) and C_abs >= -0.25)
            or (np.isfinite(R_abs) and R_abs >= -0.25)
        )
    )
    if functional_support:
        score_coact = min(1.0, score_coact + 0.14)
    if multiband_strong and np.isfinite(C_abs) and C_abs >= 0.0:
        score_coact = min(1.0, score_coact + 0.08)

    # Desacoplamiento se reserva para discordancia real entre subsistemas.
    if lcr_functional >= 0.60:
        score_disc *= 0.78

    vector_gap = V_abs - S_abs
    if (vector_gap >= 0.50 and strong_alert_spec) or (vector_gap <= -0.50 and strong_vagal_spec):
        score_disc = min(1.0, score_disc + 0.15)

    scores = {
        "Coactivación": float(score_coact),
        "Reciprocidad": float(score_recip),
        "Coinhibición": float(score_coinhib),
        "Desacoplamiento": float(score_disc),
    }
    ...
    if margin < 0.08:
        ...
        qualifier = "moderadamente definida"

    if topology == "Coactivación" and functional_support:
        qualifier = "funcional por convergencia multidimensional"

    # ------------------------------------------------------------
    # Predominio: V/S primero, pero una dominancia espectral actual FUERTE
    # puede corregir una contradicción vectorial.
    # ------------------------------------------------------------
    if V_abs >= S_abs + 0.20:
        vector_pred = "Predominio Vagal"
    elif S_abs >= V_abs + 0.20:
        vector_pred = "Predominio Alerta/Barorreflejo"
    else:
        vector_pred = "Sin predominio claro"

    predominance = vector_pred
    spectral_note = "Sin evidencia espectral transversal concluyente"
    discordance = False

    if vagal_spec and not alert_spec:
        spectral_note = "Espectro actual apoya predominio vagal"
        if vector_pred in ("Sin predominio claro", "No determinado"):
            predominance = "Predominio Vagal"
        elif vector_pred == "Predominio Alerta/Barorreflejo" and strong_vagal_spec:
            predominance = "Predominio Vagal"
            discordance = True

    elif alert_spec and not vagal_spec:
        spectral_note = "Espectro actual apoya alerta/barorreflejo"
        if vector_pred in ("Sin predominio claro", "No determinado"):
            predominance = "Predominio Alerta/Barorreflejo"
        elif vector_pred == "Predominio Vagal" and strong_alert_spec:
            predominance = "Predominio Alerta/Barorreflejo"
            discordance = True
    ...
    if coinhibition_allowed:
        gate_note = "Coinhibición permitida: V/S/R/C reducidos y sin actividad multibanda actual conservada."
    else:
        reasons = []
        if multiband_current:
            reasons.append("actividad multibanda actual conservada")
        if np.isfinite(C_abs) and C_abs >= -0.25:
            reasons.append("complejidad no reducida")
        if np.isfinite(R_abs) and R_abs >= -0.35:
            reasons.append("amplitud/reserva no suficientemente reducida")
        if strong_vagal_spec:
            reasons.append("dominancia espectral HF/vagal fuerte")
        if strong_alert_spec:
            reasons.append("dominancia espectral LF/barorrefleja fuerte")
        gate_note = "Coinhibición bloqueada por: " + (", ".join(reasons) if reasons else "falta de convergencia reductora V/S/R/C")

    return {
        "topologia": topology,
        "calificador_topologico": qualifier,
        "topologia_mostrada": topology_display,
        "predominio": predominance,
        "reserva": reserve,
        "estado": state,
        "predominio_vectorial": vector_pred,
        "apoyo_espectral": spectral_note,
        "discordancia_espectral": bool(discordance),
        "HF_DOM_actual": hfdom,
        "LF_DOM_actual": lfdom,
        "LF_HF_actual": lfhf,
        "Amplitud_absoluta_idx": idx_amp,
        "Patron_oscilatorio_actual_apoyo": osc_pat,
        "Nota_convergencia_actual": (
            "Topología multidimensional con compuerta fisiológica de coherencia. "
            f"Scores → Coactivación {score_coact:.2f}, Reciprocidad {score_recip:.2f}, "
            f"Coinhibición {score_coinhib:.2f}, Desacoplamiento {score_disc:.2f}. "
            f"{gate_note} {osc_note}"
        ),
        "Score_Coactivacion": float(score_coact),
        "Score_Reciprocidad": float(score_recip),
        "Score_Coinhibicion": float(score_coinhib),
        "Score_Coinhibicion_bruto": float(_score_coinhib_raw_v15532),
        "Score_Desacoplamiento": float(score_disc),
        "Contribucion_VS_pct": 55,
        "Contribucion_LCR_pct": 45,
        "Actividad_multibanda_actual": bool(multiband_current),
        "Actividad_multibanda_actual_fuerte": bool(multiband_strong),
        "Coinhibicion_permitida": bool(coinhibition_allowed),
        "Motivo_compuerta_coinhibicion": gate_note,
        "Band_scores_contextuales": band_scores,
    }
...
def _autonomic_plain_language_v15529(row):
    ...
    if trend == "Ganancia de Flexibilidad / Complejidad Adaptativa":
        ...
        evo = "Mejora de la reserva y recuperación"
        evo_note = "La variabilidad y la capacidad de recuperación han aumentado respecto a registros anteriores."
...
def autonomic_control_assessment(df, row, record_data=None):
    ...
    _profile_v15528 = _load_patient_reference_profile_v15528(_patient_id_v15528)
    abs_levels = _autonomic_abs_vector_levels_v15528(
        row,
    ...
    instant = _autonomic_classify_instantaneous_v15531(
        V_abs, S_abs, L_abs, C_abs, R_abs,
        hfdom=get('HF_DOM_PCT'),
        lfdom=get('LF_DOM_PCT'),
        lfhf=get('LF_HF'),
    ...
    return {
        ...
        'Contribucion_VS_topologia_pct': instant.get('Contribucion_VS_pct', 55),
        'Contribucion_LCR_topologia_pct': instant.get('Contribucion_LCR_pct', 45),
...
with tab10:
    st.subheader("Control autonómico · perfil único por paciente + coherencia fisiológica")
    st.info(
    ...
    if _auto_table_v15521.empty:
        ...
        _selected_profile_v15532 = _load_patient_reference_profile_v15528(_selected_patient_v15532)
        if _selected_profile_v15532.get('age_years') is not None:
            st.caption(
                f"Perfil aplicado automáticamente a este registro: "
                f"{_selected_profile_v15532.get('age_years')} años · "
                f"{_selected_profile_v15532.get('sex', 'No especificado')} · "
                f"paciente `{_selected_patient_v15532}`."
            )
        else:
            st.warning(
        ...
        with st.expander("Análisis avanzado · vectores, espectro y Z-scores", expanded=False):
            ...
            with st.expander("Cómo se decide la topología actual", expanded=False):
                st.write(
                    ...
                    "V/S aportan el 55% y Regulación lenta (L), Complejidad (C) y Reserva/amplitud (R) "
                    "aportan conjuntamente el 45%."
                )
                _top_scores_v15530 = pd.DataFrame({
                    "Familia": ["Coactivación","Reciprocidad","Coinhibición","Desacoplamiento"],
                    "Score": [
        ...
        with st.expander("Historial cronológico completo", expanded=False):
            ...
            st.download_button(
                ...
                file_name='control_autonomico_v15532.csv',
                mime='text/csv'
            )
